...
if __name__ == "__main__":
    ...
    if axis[1] == 'r':
        ...
        plt.ylabel(f'{axis[1]} (m)')
        plt.title(f'Respuesta ante {sig_type} para el eje {axis[1]}')
    plt.xlabel('Tiempo (s)')
    ...
    for j, log in enumerate(logger):
        ...
        u = controller[axis_ctrl[i]]
        t = controller['t']
        plt.plot(controller['t'], controller[axis[1]], '-', color=colors[j],  linewidth=2)

    plt.plot(controller['t'], controller[axis_ctrl[1]], '--', color='#ff7f0e')
    plt.legend(ctrls, loc=loc)
    plt.xlim(t[0], t[-1])
    plt.grid()
