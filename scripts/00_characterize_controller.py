...
if __name__ == "__main__":
    ...
    parser.add_argument('--drone',              default="hb",     type=str,    help='Drone model (default: CF2X)', metavar='')
    ...
    controller = logger.getStates(drone=0, states=axis+axis_ctrl+['t'])
    ...
    if ax == 'r':
        plt.ylabel(f'{ax} (rad)')
    plt.xlabel('Tiempo (s)')
    plt.title(f'Respuesta ante escalón para el eje {ax}')
    
    plt.plot(controller['t'], controller[axis[0]], '-', color='#1f77b4',  linewidth=2)
    ...
    plt.legend([axis[0]]+[axis_ctrl[0]], loc='lower right')
    if ts<0.9*(t[-1]/2):
        plt.xlim(t[0], t[-1]/2)    
    else:
        plt.xlim(t[0], t[-1])
